...
class uix {
	...
	public function enqueue_admin_stylescripts() {
		...
		if ( ! empty( $uix['modals'] ) ) {
			wp_enqueue_script(
				$this->plugin_slug . '-core-modals',
				$uix_url . 'assets/js/uix-modals' . $prefix . '.js',
				array(
					'jquery',
					'handlebars',
				),
		...
		wp_enqueue_script(
			$this->plugin_slug . '-core-admin',
			$uix_url . 'assets/js/uix-core' . $prefix . '.js',
			array(
				'jquery',
				'handlebars',
			),
	...
	public function add_settings_pages() {
		...
		foreach ( (array) $this->pages as $page_slug => $page ) {
			...
			add_action(
				'admin_print_styles-' . $this->plugin_screen_hook_suffix[ $page_slug ],
				array(
					$this,
			...
			add_action(
				'load-' . $this->plugin_screen_hook_suffix[ $page_slug ],
				array(
					$this,
					'add_help',
				)
				...
				foreach ( (array) $uix['tabs'] as $tab_slug => $tab ) {
					...
					<script type="text/html"
						...
						if ( ! empty( $tab['page_description'] ) ) {
							?>
							<small><?php echo $tab['page_description']; ?></small> 
											  <?php
						}
						if ( ! empty( $tab['page_title'] ) ) {
							echo '</h2>';
