CMB.addCallbackForDeletedField(
	'CMB_wysiwyg',
	function( el ) {

		// Destroy WYSIWYG editors instances.
		el.find( '.cmb-wysiwyg textarea' ).each(
			function() {
	...
	function( newT ) {
		...
		newT.find( '.cmb-wysiwyg' ).each(
			function (i) {
				...
				try {
					quicktags( tinyMCEPreInit.qtInit[id] ); } catch (e) {
					}
...
CMB.addCallbackForSortStart(
	'CMB_wysiwyg',
	function( el ) {

		el.find( '.wp-editor-area' ).each(
			function(){
